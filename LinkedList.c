	...
	while(pFreeNode) 
	{
		if(pFreeNode->sizeOfNodeInBytes > requestedBytes)
		{
			...
			if(!pUsedList)
			{
				...
				pUsedList->memPtr = pFreeNode->memPtr;
				pUsedList->sizeOfNodeInBytes = requestedBytes;
				pUsedList->nextListPtr = NULL;*/
				pUsedList = (node *)malloc(sizeof(node));
				pUsedList = (node*) UsedNode;
			}
			else
			{
				node * pUsedNode = pUsedList;
				while(pUsedNode->nextListPtr) //Go to end of used linked list
				{
					pUsedNode = pUsedNode->nextListPtr;
				}
				pUsedNode->nextListPtr = UsedNode;
			}
